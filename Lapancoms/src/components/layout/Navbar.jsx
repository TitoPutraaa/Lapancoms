...
export default function Navbar() {
  ...
  // useEffect(() => {
  //   console.log("isScrollDown berubah:", isScrollDown);
  // }, [isScrollDown]);
  ...
  const navTextClass = !isScrollDown && !isAtTop ? "text-dark" : "text-white";
  return (
    <div
      className={`fixed top-0 left-0 z-100 w-full transition-transform duration-500 ${isScrollDown && isSm ? "-translate-y-full" : "translate-y-0"}`}
    >
      <nav className={`${navBgClass} transition-all duration-300`}>
        <div className="relative container mx-auto flex flex-wrap items-center justify-between px-4 py-4 lg:px-8">
          ...
          <div
            className={`fixed top-0 right-0 bottom-0 w-0 transition-all duration-600 sm:static sm:h-auto sm:w-fit sm:bg-transparent ${isOpen ? "h-screen w-full bg-gray-100" : "bg-transparent"}`}
          >
            <div
            ...
            >
              <a className="text-dark flex items-center space-x-3">
