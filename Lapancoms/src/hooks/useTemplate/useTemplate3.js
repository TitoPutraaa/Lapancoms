...
export default function useTemplate3() {
  ...
  const [text3, setText3] = useState(() => {
    const saved = localStorage.getItem("prev_text3");
    return saved ? JSON.parse(saved) : null;
  });
  ...
  const handleChangeText3 = (content, delta, source, editor) => {
    ...
    const deltaValue = editor.getContents();
    setText3(deltaValue);
    localStorage.setItem("prev_text3", JSON.stringify(deltaValue));
  };
  ...
  const handleFileChange3 = (e) => {
    ...
    reader.onloadend = () => {
      ...
      localStorage.setItem("prev_img3", reader.result);
    };
    reader.readAsDataURL(file);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    formData.append("text3", JSON.stringify(text3));
